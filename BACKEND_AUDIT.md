...
- [x] One-off cleanup migration for already-orphaned lectures/assignments and dangling `modules[]` ids — 2026-09-12. Added `backend/scripts/cleanup-orphaned-module-content.js` (dry-run by default, `--apply` to actually delete/clean). **Dry run against the real dev database found real orphans from the historical bug: 13 orphaned lectures, 5 orphaned assignments, 3 courses with dangling `modules[]` ids.** Not applied yet — pending the user's go-ahead, since it deletes documents and Cloudinary assets.
